...
//OBJECT: 0 MATERIAL NO. 0 'player_missile'OK 
	...
	gDPSetRenderMode(glistp++, G_RM_FOG_SHADE_A, G_RM_RA_ZB_OPA_SURF2);
...
gSPSetLights2(glistp++, projectileLight);
 ...
 void spawn_player_shot(ProjectileObject* projectile, GameObject* targetObject)
 {
	...
	SetVector3(&bulletSpawnPos,	playerAvatar.sphereCollider.center.x + playerAvatar.animRig.joint.armR[1].pos.x, playerAvatar.sphereCollider.center.y + playerAvatar.animRig.joint.armR[1].pos.y, playerAvatar.sphereCollider.center.z + playerAvatar.animRig.joint.armR[1].pos.z);
	...
	//SetVector3(&projectile->obj.pos, playerAvatar.animRig.joint.armR[1].pos.x + tempVelocity.x,playerAvatar.animRig.joint.armR[1].pos.y + tempVelocity.y ,playerAvatar.animRig.joint.armR[1].pos.z + 10.0f + tempVelocity.z);
	SetVector3(&projectile->obj.pos, bulletSpawnPos.x +tempVelocity.x, bulletSpawnPos.y + tempVelocity.y, bulletSpawnPos.z - 20.0f + tempVelocity.z);
	...
	if(enemyTargets.isActive == 1) 
	{
		...
		tempVelocity = vectorScale(tempVelocity, 60.0f);
	}
	...
	{		
        ...
        tempVelocity.x = playerAvatar.directionNormal.x;
        tempVelocity.y = playerAvatar.directionNormal.y;
        VectorNormalize(&tempVelocity);
        tempVelocity = vectorScale(tempVelocity, 60.0f);
