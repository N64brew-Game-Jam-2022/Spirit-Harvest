...
Gfx          gfx_clear_glist[3][GFX_CLEAR_GLIST_LEN];
...
gfxClearCfb(void)
{
    // Clear the Z-buffer  
    /*
    if(interlaceSwitch == 0)
    {
        gDPSetScissor(glistp++,G_SC_ODD_INTERLACE, 0,0, SCREEN_WD,SCREEN_HT); //interlace for graphics...? Might work for some graphical effects but does not improve performance
        interlaceSwitch = 1;
    }
    
    else if(interlaceSwitch == 1)
    {
        gDPSetScissor(glistp++,G_SC_EVEN_INTERLACE, 0,0, SCREEN_WD,SCREEN_HT); //interlace for graphics...? Might work for some graphical effects but does not improve performance
        interlaceSwitch = 0;
    ...
    */
	...
	//gDPSetScissor(glistp++,G_SC_NON_INTERLACE, 0,0, SCREEN_WD,SCREEN_HT); //non-interlace for black
	//BLACK FRAME BUFFER
       // gDPSetFillColor(glistp++, (GPACK_RGBA5551(15,55,60, 1) << 16 | 
			      // GPACK_RGBA5551(15,55,60, 1)));
    
   //CUSTOM FRAME BUFFER
    gDPSetFillColor(glistp++, (GPACK_RGBA5551(viewSettings.backgroundColor.r,viewSettings.backgroundColor.g,viewSettings.backgroundColor.b, 1) << 16 | 
			      GPACK_RGBA5551(viewSettings.backgroundColor.r,viewSettings.backgroundColor.g,viewSettings.backgroundColor.b, 1)));
   // gDPSetFillColor(glistp++, (GPACK_RGBA5551(u8(viewSettings.backgroundColor.x),int(viewSettings.backgroundColor.y),int(viewSettings.backgroundColor.z), 1) << 16 | 
			      // GPACK_RGBA5551(viewSettings.backgroundColor.x,viewSettings.backgroundColor.y,viewSettings.backgroundColor.z, 1)));
				   
	// BRIGHT YELLOW BUFFER
	// gDPSetFillColor(glistp++, (GPACK_RGBA5551(255, 230, 200, 1) << 16 | 
			       // GPACK_RGBA5551(255, 230, 200, 1)));
...
void RenderStaticEnvObj(StaticEnvObject* env)
{ 
		gDPPipeSync(glistp++);
		...
		//env->animator.animate(env->animator.animFrame);
        gSPDisplayList(glistp++, env->staticModel.displayList);
...
{ 
        ...
        //if(*enemy->*animator2 != NULL)
        enemy->animator2.animate(enemy->animator.animFrame);
...
void PlayerEffectMTX(RigObject* rigObj, void (*rigFunc)(void))
{ 
		//For storing current player model's position, rotation and scale in matrices 
		gDPPipeSync(glistp++);
		gSPClearGeometryMode(glistp++,0xFFFFFFFF);
		...
		guTranslate(&rigObj->matrix[0], rigObj->pos.x, 
										rigObj->pos.y, 
										rigObj->pos.z);
...
void PlayerSkinnedObjectMTX(Player* pla)
{ 
