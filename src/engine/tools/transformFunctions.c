...
void SetAnimator(Animation *setAnim, void (*setAnimFunc)(u8), u8 defaultAnimID, u8 defaultAnimFrame)
...
void SetStaticDL(StaticModel *setModel, Gfx* setStaticModel)
{	//NOTE: For frame by frame animation, morphing, etc 
//StaticModel staticModel; //pointer to display List
	setModel->displayList = setStaticModel;
	//setAnim->animID = defaultAnimID;
	//setAnim->animFrame = defaultAnimFrame;
}
...
float angleFromVectors(Vector3 startPoint, Vector3 pointB, Vector3 pointC)
{
	...
	// Vector3 playerToC = vectorSubtract(vertexC.vertex, startPoint);
     ...
     float slopeA = (pointB.y - startPoint.y) / (pointB.x - startPoint.x);
     float slopeB = (startPoint.y - pointC.y) / (startPoint.x - pointC.x);
     float angle = atan2f(abs((slopeB - slopeA) / (1+ (slopeA*slopeB))));
     
     return angle;
     
    // Vector3 slopeB = (pointC.y - pointA.y) / (pointC.x - pointA.x);
}
...
float vectorAngleTriangle(Vector3 pointA, Vector3 pointB, Vector3 pointC)
{
	...
	// float den = sqrtf(powf((pointB.x-pointA.x), 2) + powf((pointB.y-pointA.y),2) + powf((pointB.z-pointA.z),2))/
        ...
        float denA = sqrtf(((pointB.x-pointA.x)*(pointB.x-pointA.x)) + ((pointB.y-pointA.y)*(pointB.y-pointA.y)) + ((pointB.z-pointA.z)*(pointB.z-pointA.z)));
        float denB = sqrtf(((pointC.x-pointA.x)*(pointC.x-pointA.x)) + ((pointC.y-pointA.y)*(pointC.y-pointA.y)) + ((pointC.z-pointA.z)*(pointC.z-pointA.z)));
		//sqrtf(powf((pointC.x-pointA.x), 2) + powf((pointC.y-pointA.y),2) + powf((pointC.z-pointA.z),2));
        
        
       // float denA = sqrtf(powf((pointB.x-pointA.x), 2) + powf((pointB.y-pointA.y),2) + powf((pointB.z-pointA.z),2));
        //float denB = sqrtf(powf((pointC.x-pointA.x), 2) + powf((pointC.y-pointA.y),2) + powf((pointC.z-pointA.z),2));
        float den = denA / denB;
        
	//float	den = 5.0f;
	float angle = acosf(num / den)*(180.0/ PI);
    //float angle = acosf(num / den)*57.29577;
    
	return angle;
}

float vectorAngle(Vector3 pointA, Vector3 pointB)
{
    float deltaX = pointA.x - pointB.x;
    float deltaY = pointA.y - pointB.y;
    float radians = atan2f(deltaY, deltaX);
    float angle = (radians * 57.29577);
    return radians;
}
